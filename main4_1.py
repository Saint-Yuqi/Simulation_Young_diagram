...
def single_iteration_calculation(n):
    # 由于需要独立计算每个迭代的 u 值，这个函数独立处理单次迭代
...
def calculate_u_values_over_iterations_parallel(num_iterations, n):
    # 使用多进程池并行处理迭代
    with Pool() as pool:
        u_values = list(tqdm(pool.imap(single_iteration_calculation, [n] * num_iterations), total=num_iterations))
    return u_values
...
u_values_parallel = calculate_u_values_over_iterations_parallel(num_iterations, n)
close_count = count_close_u_values(u_values_parallel, epsilon=0.1)
...
# Plot the u values
plot_u_values(u_values_parallel, n, num_iterations)